...
describe("Points", () => {
    ...
    describe("constructor", () => {
        ...
        describe("when settings.data is a FeatureCollection", () => {
            it('sets this.dataFormat to "Array"', () => {
                ...
                expect(getPoints({
                    data,
                }).dataFormat).toEqual("GeoJson.FeatureCollection");
            });
        });
        ...
        describe("when the map projector is not SphericalMercator", () => {
            ...
            it("warns", () => {
                ...
                ((_a = map.options.crs) !== null && _a !== void 0 ? _a : { code: "" }).code = "123";
                getPoints({ map });
                expect(warnSpy).toHaveBeenCalledWith("layer designed for SphericalMercator, alternate detected");
            });
        });
        it("calls this.setup", () => {
            getPoints();
    ...
    describe("render", () => {
        ...
        it("sets this.mapMatrix size to canvas", () => {
            ...
            points.render();
            expect(setSizeSpy).toHaveBeenCalledWith(points.canvas.width, points.canvas.height);
        });
        it("calls this.gl.viewport correctly", () => {
            const points = getPoints();
            const viewportSpy = jest.spyOn(points.gl, "viewport");
            points.render();
            expect(viewportSpy).toHaveBeenCalledWith(0, 0, points.canvas.width, points.canvas.height);
        });
        it("calls this.gl.uniformMatrix4fv correctly", () => {
            const points = getPoints();
            const uniformMatrix4fvSpy = jest.spyOn(points.gl, "uniformMatrix4fv");
            points.render();
            expect(uniformMatrix4fvSpy).toHaveBeenCalledWith(points.matrix, false, points.mapMatrix.array);
        });
        it("calls this.gl.bindBuffer correctly", () => {
            const points = getPoints();
            const bindBufferSpy = jest.spyOn(points.gl, "bindBuffer");
            points.render();
            expect(bindBufferSpy).toHaveBeenCalledWith(points.gl.ARRAY_BUFFER, points.buffers.vertices);
        });
        it("calls this.gl.bufferData correctly", () => {
            const points = getPoints();
            const bufferDataSpy = jest.spyOn(points.gl, "bufferData");
            points.render();
            expect(bufferDataSpy).toHaveBeenCalledWith(points.gl.ARRAY_BUFFER, points.typedVertices, points.gl.STATIC_DRAW);
        });
        it("calls this.attachShaderVariables correctly", () => {
            const points = getPoints();
            const attachShaderVariablesSpy = jest.spyOn(points, "attachShaderVariables");
            points.render();
    ...
    describe("resetVertices", () => {
        ...
        describe('when this.dataFormat is "Array"', () => {
            ...
            it("projects", () => {
                ...
                expect(points.dataFormat).toBe("Array");
                const project = (points.map.project = jest.fn((latlng, zoom) => {
            ...
            it("calls this.addLookup with vertex", () => {
                ...
                points.map.project = jest.fn((latlng, zoom) => {
                    return new Point(0, 1);
                });
                points.resetVertices();
                const expected = {
            ...
            describe("when this.settings.eachVertex is defined", () => {
                it("is called with vertex", () => {
                    ...
                    const size = 6;
                    const eachVertex = jest.fn((vertex) => { });
    ...
    describe("drawOnCanvas", () => {
        ...
        describe("when this.gl is truthy", () => {
            ...
            it("calls this.gl correctly", () => {
                ...
                expect(uniformMatrix4fvSpy).toHaveBeenCalledWith(matrix, false, mapMatrix.array);
                expect(drawArraysSpy).toHaveBeenCalledWith(gl.POINTS, 0, allLatLngLookup.length);
            });
        });
    });
    describe("lookup", () => {
        ...
        describe("when matches", () => {
            it("calls Points.closest with coords and matches", () => {
                ...
                expect(closestSpy).toHaveBeenCalledWith(coords, [pointVertex], points.map);
            });
        });
        describe("when not matches", () => {
            it("calls Points.closest with coords and matches", () => {
                ...
                const coords = new LatLng(0, 0);
                points.lookup(coords);
                expect(closestSpy).toHaveBeenCalledWith(coords, points.allLatLngLookup, points.map);
            });
        });
    });
    // TODO: tryClick
...
function getPointsWithColor(color) {
    const element = document.createElement("div");
    const map = new Map(element);
...
function getPointsWithColorFunction(colorFn) {
    const element = document.createElement("div");
    const map = new Map(element);
    const data = [
        [50.2, 14.2],
...
describe("Points - Color Function", () => {
    it("applies the correct color from the color function", () => {
        ...
        const colorFunction = (index) => color_min[index];
    ...
    it("handles invalid color function gracefully", () => {
        const invalidColorFunction = (index) => null;
