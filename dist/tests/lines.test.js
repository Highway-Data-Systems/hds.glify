...
describe("Lines", () => {
    ...
    describe("render", () => {
        ...
        beforeEach(() => {
            ...
            getAttributeLocationSpy = jest.spyOn(Lines.prototype, "getAttributeLocation");
        });
        afterEach(() => {
        ...
        it("calls gl.bindBuffer correctly", () => {
            ...
            lines.render();
            expect(bindBufferSpy).toHaveBeenCalledWith(lines.gl.ARRAY_BUFFER, vertexBuffer);
        });
        it("calls gl.bufferData correctly", () => {
            ...
            lines.render();
            expect(bufferDataSpy).toHaveBeenCalledWith(gl.ARRAY_BUFFER, lines.allVerticesTyped, gl.STATIC_DRAW);
        });
        it("calls gl.vertexAttribPointer correctly", () => {
            ...
            lines.render();
            expect(vertexAttribPointerSpy).toHaveBeenCalledWith(42, 2, gl.FLOAT, false, 24, 0);
        });
        it("calls gl.enableVertexAttribArray correctly", () => {
            const lines = new Lines(settings);
            getAttributeLocationSpy.mockReturnValue(42);
            const enableVertexAttribArraySpy = jest.spyOn(lines.gl, "enableVertexAttribArray");
            lines.render();
        ...
        it("calls mapMatrix.setSize() correctly", () => {
            ...
            lines.render();
            expect(setSizeSpy).toHaveBeenCalledWith(lines.canvas.width, lines.canvas.height);
        });
        it("calls gl.viewport() correctly", () => {
            ...
            lines.render();
            expect(viewportSpy).toHaveBeenCalledWith(0, 0, lines.canvas.width, lines.canvas.height);
        });
        it("calls gl.uniformMatrix4fv() correctly", () => {
            ...
            lines.render();
            expect(uniformMatrix4fvSpy).toHaveBeenCalledWith(lines.matrix, false, lines.mapMatrix.array);
        });
        it("calls this.attachShaderVariables() correctly", () => {
            const lines = new Lines(settings);
            const attachShaderVariablesSpy = jest.spyOn(lines, "attachShaderVariables");
            lines.render();
    ...
    describe("resetVertices", () => {
        ...
        it("accumulates this.vertices, this.allVertices and this.allVerticesTyped correctly", () => {
            ...
            expect(lines.vertices[0].settings.longitudeKey).toEqual(lines.longitudeKey);
            ...
            expect(lines.vertices[0].settings.opacity).toEqual(lines.settings.opacity);
    ...
    describe("drawOnCanvas", () => {
        function callDrawOnCanvas(lines, event) {
            lines.drawOnCanvas({
                canvas: lines.canvas,
    ...
    describe("tryClick", () => {
        ...
        beforeEach(() => {
            ...
            const latlngArray = mockFeatureCollection.features[0].geometry.coordinates[0];
        ...
        describe("when map is same", () => {
            ...
            describe("when a feature is near point", () => {
                it("calls settings.click", () => {
                    Lines.tryClick(mockClick, lines.map, [lines]);
                    expect(lines.settings.click).toHaveBeenCalledWith(mockClick, lines.data.features[0]);
                });
            ...
            describe("when a feature is near point", () => {
                describe("when lines.hoverFeatures does not contain the feature being hovered", () => {
                    ...
                    it("calls settings.hover", () => {
                        Lines.tryHover(mockHover, lines.map, [lines]);
                        expect(hoverMock).toHaveBeenCalledWith(mockHover, lines.data.features[0]);
                    });
                });
                describe("when a feature is not near point", () => {
                    // distance = 2
...
function getLines(settings) {
    const lineSettings = getSettings(settings);
    return new Lines(lineSettings);
}
